...
import fuzs.diagonalblocks.mixin.client.accessor.*;
...
public class MultipartAppender {
    ...
    private static void appendNewSelector(BiConsumer<ResourceLocation, UnbakedModel> modelAdder, Condition newCondition, Selector selector, EightWayDirection direction, List<Selector> newSelectors) {
        ...
        for (Variant variant : variants) {
            ...
            ResourceLocation resourceLocation = variant.getModelLocation().withPath((String path) -> path + "_" + interDirection.getSerializedName());
            ...
            newVariants.add(new Variant(resourceLocation, variant.getRotation(), variant.isUvLocked(), variant.getWeight()));
        }
